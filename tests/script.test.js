...
describe('SailPoint IdentityNow Disable Account Action', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('invoke handler', () => {
    it('should successfully disable an account', async () => {
      ...
      const mockResponse = {
        ok: true,
        json: async () => ({ id: 'task789', message: 'Account disable initiated' })
      };
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://example.api.identitynow.com/v3/accounts/acc123/disable',
        {
          method: 'POST',
          headers: {
    ...
    it('should handle API error responses', async () => {
      ...
      await expect(script.invoke(params, context))
        .rejects.toThrow('Failed to disable account: NOT_FOUND - track123');
    });
    ...
    it('should handle non-JSON error responses', async () => {
      ...
      await expect(script.invoke(params, context))
        .rejects.toThrow('Failed to disable account: Internal Server Error');
    });
    ...
    it('should encode accountId to prevent injection', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://example.api.identitynow.com/v3/accounts/acc%2F123%26test%3D1/disable',
        expect.any(Object)
      );
    });
  });
  ...
  describe('error handler', () => {
    ...
    it('should throw on unrecoverable errors', async () => {
      ...
      await expect(script.error(params, context))
        .rejects.toThrow('Unrecoverable error disabling account acc123: Authentication failed');
    });
  });
